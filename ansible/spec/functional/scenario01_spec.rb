...
describe command("python wrapper.py run-feeder #{property['test_account']} #{property['test_password']} #{property['controlserver_url']} dummy") do
  its(:stdout) { should contain("created") }
  its(:exit_status) { should eq 0 }
end

describe command("python wrapper.py run-agent #{property['test_account']} #{property['test_password']} #{property['controlserver_url']} helloworld #{property['test_remoteci_name']} #{property['test_account']}") do
...
describe command("python wrapper.py file-list #{property['test_account']} #{property['test_password']} #{property['controlserver_url']}") do
  its(:stdout) { should contain("Helloworld") }
  its(:exit_status) { should eq 0 }
end
